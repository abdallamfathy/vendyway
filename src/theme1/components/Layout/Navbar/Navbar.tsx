...
const Navbar: React.FC = () => {
  ...
  return (
    ...
    <div className="container mx-auto py-4 max-md:p-4">
            <div className="flex justify-between items-center">
                <div className='flex items-center gap-5'>
                    <RiMenu2Fill size={24} className="md:hidden" onClick={handleShowMenu}/>
                ...
                <div className="flex items-center gap-5">
                    ...
                    <Link to='/cart'><img src={shopBag} alt="shopBag" /></Link>
                    <button className="max-md:hidden bg-pink1 text-white rounded-full p-2 px-5 hover:bg-orange-500 transition-colors duration-400 ease-in ">Sign In</button>
                    <BsPerson size={30} className="md:hidden"/>
                </div>
            </div>
            ...
            <div className="md:flex hidden justify-center items-center md:gap-20 gap-5  bg-pink1 text-[#EBEBEB] font-medium py-4 ">
                {
                    Navigation.map((nav, index) => {
                        return (
                            <NavLink key={index} to={nav.link}>{nav.title}</NavLink>
                        )
                    })
                }
            </div>
    ...
    <div id="drawer-example" className={`fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform bg-gray1 w-80  ${showMenu ? '' : '-translate-x-full'}`} tabIndex={-1} aria-labelledby="drawer-label">
       ...
       <button type="button"  onClick={handleShowMenu} className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-2.5 right-2.5 inline-flex items-center justify-center border border-lgray1" >
